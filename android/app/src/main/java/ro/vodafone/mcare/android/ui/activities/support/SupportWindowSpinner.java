...
public class SupportWindowSpinner extends AutoResizeTextView {
    ...
    private SupportSpinnerPopupWindow popupWindow;
    ...
    private boolean isValueChoosen;
    private String tag;


    @SuppressWarnings("ConstantConditions")
    public SupportWindowSpinner(Context context) {
        super(context);
        init(context, null);
    }

    public SupportWindowSpinner(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public SupportWindowSpinner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void onRestoreInstanceState(Parcelable savedState) {
        if (savedState instanceof Bundle) {
            ...
            int index= bundle.getInt(SELECTED_INDEX);

            if (adapter != null && index != -1) {
                setSelectedIndex(index);
                enableSpinner();
                //setText(adapter.getItem(infoSelectedIndex).toString());
                //adapter.notifyItemSelected(infoSelectedIndex);
                setValueChoosen(true);
            }
...
//                        D.w("listView size = " + realSize);
        ...
        supportWindow = VodafoneController.getInstance().supportWindow(VodafoneController.currentActivity());
        popupWindow = new SupportSpinnerPopupWindow(supportWindow.getRootLayout());
        ...
        popupWindow.setOnDismissListener(new SupportSpinnerPopupWindow.OnDismissListener() {
    ...
    /**
     * Note: Don't use separately. Use setSelectedIndex(int position), because when value is set the index should be updated.
     * @param value
     */
    ...
    private void setAdapterInternal(@NonNull BaseAdapter adapter) {
        ...
        //setText(adapter.getItemInDataset(infoSelectedIndex).toString());
...
//        D.w("event = " + event.getAction());
    ...
    public void showDropDown() {
        ...
        popupWindow.showAtLocation(supportWindow.activity.getWindow().getDecorView(), Gravity.TOP | Gravity.START, location[0], location[1] + getHeight() + supportWindow.getStatusBarHeight());
        popupWindow.setDismissOnTouchOutside(true);
    ...
    private void enableSpinner(){
        setClickable(true);
        setFocusable(true);
        setBackground(ContextCompat.getDrawable(getContext(), R.drawable.default_input_border));
    ...
    /**
     *
     * @return true if has a selected value, and false if doesn't.
     */
    public boolean isValueChoosen() {
        return isValueChoosen;
    }
     ...
     * @param valueChoosen
     */
    public void setValueChoosen(boolean valueChoosen) {
        isValueChoosen = valueChoosen;
    }
