...
public class CartesianView extends View {
    ...
    float p_x_label_text_size = dipToPixel(7.0f);
    float p_y_label_text_size = dipToPixel(7.0f);
     ...
     */
    public void setTextStyle(int color,float xLabelSize, float yLabelSize) {
        p_text_color = color;
        p_x_label_text_size = dipToPixel(xLabelSize);
        p_y_label_text_size = dipToPixel(yLabelSize);
        initPaint();
        bRedraw = true;
        postInvalidate();
    }
    ...
    protected void initPaint() {
        ...
        //mPntText.setTextSize(p_y_label_text_size);
    ...
    protected void getViewSizes() {
        ...
        if (p_ytext_vis &&  p_ytext_left)   sX += 3.3*p_y_label_text_size;
        if (p_ytext_vis && !p_ytext_left)   eX -= 3*p_y_label_text_size;
        if (p_xtext_vis &&  p_xtext_bottom) eY -= p_x_label_text_size+dipToPixel(10);
        if (p_xtext_vis && !p_xtext_bottom) sY += p_x_label_text_size+dipToPixel(10);
        // chart area range
        dX = eX - sX;
        dY = eY - sY;
    }
    ...
    protected void calcYgridRange() {
        ...
        //Todo need more testing this hack
        //mYminGrid param will be alway 0.
        mYminGrid = 0;
    ...
    protected void drawGrid() {
        ...
        for (ii=1;ii<=mYgridNum;ii++) {
            mPath.moveTo(sX,eY - ((mAbsoluteYMax * aY) / mYgridNum) * ii - mBarWidth - bY);
            mPath.lineTo(eX,eY - ((mAbsoluteYMax  * aY) / mYgridNum) * ii - mBarWidth - bY);
        }
    ...
    protected void drawXlabel() {
        ...
        if (p_xtext_bottom) {
            for (ii=1;ii<mXgridNum;ii++) {
                ...
                mCnv.drawText(String.format("%.1f",ff),sX+ii*(dX/mXgridNum),eY+p_x_label_text_size+2,mPntText);
            }
        } else {
            for (ii=1;ii<mXgridNum;ii++) {
                ...
                ff = mXminGrid + ii*(mXmaxGrid-mXminGrid)/mXgridNum;
                mCnv.drawText(String.format("%.1f",ff),sX+ii*(dX/mXgridNum),sY-p_x_label_text_size+2,mPntText);
            }
        }
    ...
    protected void drawYlabel() {
        ...
        mPntText.setTextSize(p_y_label_text_size);
        mPath.reset();
        TextUtils.setTextSizeForWidth(mPntText, ScreenMeasure.dpToPx(70), String.valueOf(mAbsoluteYMax) + " " + unit);
        ...
        if (p_ytext_left) {
            for (ii=1;ii<=mYgridNum;ii++) {
                ...
                mCnv.drawText(String.valueOf((int)((mAbsoluteYMax/mYgridNum) * ii))+ " " + unit,sX-6,eY - ((mAbsoluteYMax * aY / mYgridNum) * ii + mBarWidth / 2 - bY) + p_y_label_text_size/3 ,mPntText);
            }
        } else {
            for (ii=1;ii<mYgridNum;ii++) {
                ...
                ff = mYminGrid + ii*(mYmaxGrid-mYminGrid)/mYgridNum;
                mCnv.drawText(String.valueOf((int)((mAbsoluteYMax/mYgridNum) * ii)) + " " + unit,eX+6,eY - ((mAbsoluteYMax * aY / mYgridNum) * ii + mBarWidth / 2 - bY) + p_y_label_text_size/3 ,mPntText);
            }
        }
        mCnv.drawPath(mPath,mPntAxis);
    }
